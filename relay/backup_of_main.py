...
'''
# !: This should be implemented as a local package instead!
import sys
# sys.path.append('C:\\Users\\mikke\\OneDrive - Aalborg Universitet\\Semester 2\\project\\code\\better\\P2-Remote-inspection-and-exploration-using-camera-drone\\client')
sys.path.append(
    '/home/mikkel/OneDrive_TD33XS@student.aau.dk/Semester 2/project/code/better/P2/client'
)
...
class Relaybox:
    ...
    def __init__(self, name: str, password: str) -> None:
        ...
        # Used for the heartbeat function only. Token did not work
    ...
    def backend_authentication(self) -> None:
        ...
        try:
            ...
            if response.status_code != 200:
                logging.error(
                    f'Tried connecting to {response.url} | {response.status_code} | Reconnecting in 2 seconds')
                sleep(2)
        ...
        # When no reponse is given try again.
            ...
            logging.warning(
                f'Tried connecting to backend | Reconnecting in 2 seconds')
            sleep(2)
            self.backend_authentication()
    ...
    def start(self) -> None:
        ...
        scan_for_drone_thread = threading.Thread(
            target=self.scan_for_drone, args=())
        ...
        while True:
            ...
            try:
                ...
                response = self.session.get(
                    HEARTBEAT_URL, headers=self.header, json=query, timeout=10.0).json()
            ...
            logging.info(
                f'Heartbeat | {backend_data_status} | Response time: {response_time:.3f} seconds | {self.NAME}: {self.drones.keys()}')
    ...
    def backend_data_up_to_date(self, response: dict) -> str:
        ...
        for drone_name, drone_data in self.drones.items():
            ...
            backend_drone_data = response.get(self.NAME, {}).get(
                'drones', {}).get(drone_name, {})
            relay_drone_data = {'name': drone.name,
                                'ports': {'video': drone.video_port}}
        ...
        backend_drones_data: dict = response.get(
            self.NAME, {}).get('drones', {}).items()
    ...
    def disconnected_drone(self, drone: object) -> None:
        ...
        response = requests.post(
            f'{BACKEND_URL}/drone/disconnected', json=query)
...
class Drone:
    ...
    def start(self) -> None:
        ...
        if self.drone_active:
            ...
            landing_thread = threading.Thread(
                target=self.landing_thread).start()
    ...
    def landing_thread(self) -> None:
        ...
        while self.drone_active:
            ...
            should_land = requests.get(
                f'{BACKEND_URL}/drone/should_land', json=land_query)
            sleep(0.1)
            ...
            if '<Response [200]>' == str(should_land):
                ...
                status_message = requests.post(
                    f'{BACKEND_URL}/drone/successful_land', json=query)
    ...
    def rc_thread(self) -> None:
        ...
        while self.drone_active and (self.takeoff == False):
            ...
            try:
                should_takeoff = requests.get(
                    f'{BACKEND_URL}/drone/should_takeoff', json=takeoff_query)
            ...
            # If it fails to do so
                ...
                # Wait and repeat.
                logging.error(
                    f'Could not reach {BACKEND_URL}/drone/should_takeoff endpoint, retrying in 2 seconds.')
                sleep(2)
                self.rc_thread()
            ...
            if '<Response [200]>' == str(should_takeoff):
                ...
                should_takeoff = requests.post(
                    f'{BACKEND_URL}/drone/successful_takeoff', json=takeoff_query)
            ...
            while self.drone_active and self.takeoff:
                ...
                rc_commands: dict = requests.get(
                    f'{BACKEND_URL}/cmd_queue', json=command_queue)
